...
import static printPackage.printAdm.stdCB;
...
public class reportCardE extends javax.swing.JFrame {
    ...
    static int[] colWiseSum;
    static int[] rowWiseSum;
    ...
    String val="ERROR";
    if(value>9 && value<=10){val="A1";} 
           if(value>8 && value<=9){val="A2";}
           if(value>7 && value<=8){val="B1";}
           if(value>6 && value<=7){val="B2";}
           if(value>5 && value<=6){val="C1";}
           if(value>4 && value<=5){val="C2";}
           if(value>3 && value<=4 ){val="D";}
           if(value>2 && value<=3 ){val="E1";}
           if(value<=2 ){val="E2";}    
    return(val);
    }
    ...
    public static void studentShuffler(int i){
        ...
        tab=(DefaultTableModel)obj.table.getModel();
         obj.sessionCB.setSelectedItem(sessionCB.getSelectedItem());
...
//JOptionPane.showMessageDialog(null, sessTF);
    ...
    public static void grandT(DefaultTableModel t) {
      ...
      //  JOptionPane.showMessageDialog(null, t.getRowCount());
...
if(j==4){continue;}        //since third term is not wanted        
                ...
                if (value !=(-100)) {
                    ...
                    if(value!=(-1)&& value!=(-2)){sumInRow += value;System.out.println("i---j----examlimit+1-------value"+i+"------"+j+"------"+(examLimit+1)+"-----"+value);}
           ...
           // System.out.println("COMP SUB ASSIGN : "+compSub[tempRowAssign]);
            ...
            try{rowWiseSum[tempRowAssign]=sumInRow;//row wise that is subject wise sum , it is enclosed in try block because if tempRowAssign go out of bounds there will be now error
    ...
    public class priPan extends JPanel{
       ...
       gd.drawLine(2 , 42/*initial ypos*/,1048 , 42);
       gd.setColor(Color.lightGray);
       gd.fillRect(2,2,1048,40);
       gd.setColor(Color.black);
       
       //horizontal line
       int i=0;
       int sub=actualSub-noOfOpSub;//no. of actual sub
       int ypos=42;//42 here is the ypos of the above line
       ypos+=cellwidth;
...
RenderingHints.VALUE_ANTIALIAS_ON);
       Font f1=new Font("Adobe Hebrew",Font.BOLD,22);
       gd.setFont(f1);       
       int initialRowPos=60;
       int verAlign=20;
       gd.drawString("SUBJECTS", initialRowPos,verAlign);
       gd.drawString("First Term", initialRowPos+=230,verAlign);
       gd.drawString("Second Term", initialRowPos+=180,verAlign);
      // gd.drawString("Third Term", initialRowPos+=160,verAlign);
       gd.drawString("Half Yearly", initialRowPos+=210,verAlign);
       gd.drawString("GRAND TOTAL", initialRowPos+=181,verAlign);       
       Font f2=new Font("Adobe Hebrew",Font.PLAIN,18);
       gd.setFont(f2);
       verAlign+=17;
       initialRowPos=235;
       gd.drawString("Min. = 04          Max. = 10", initialRowPos,verAlign);
       gd.drawString("Min. = 04          Max. = 10", initialRowPos+=200,verAlign);
      // gd.drawString("Min. = 04               Max. = 10", initialRowPos+=200,verAlign);
      if(gt==90){gd.drawString("Min. = 24          Max. = 70", initialRowPos+=200,verAlign);}
       else{gd.drawString("Min. = 27          Max. = 80", initialRowPos+=200,verAlign);}
       if(gt==90){gd.drawString("Min. = 30             Max. = "+gt, initialRowPos+=200,verAlign);}
       else{gd.drawString("Min. = 33           Max. = 100", initialRowPos+=200,verAlign);}
      ...
      int initialverAlign=verAlign;
       ...
       if(rowWiseSum[i]<minGT){passL.setText("FAIL");    
       status="[F]";
       
       }
       sum+=rowWiseSum[i];
       gd.drawString(rowWiseSum[i]+status, 930, verAlign+=cellwidth);
       status="";
       i++;
       }
       gd.drawString(sum+"", 930, verAlign+=cellwidth);//entering total sum of vertical grand total column
       
       
       //getting percentage
       double per=(double)(sum*100)/(double)(rowWiseSum.length*gt);       
       String percentage=per+"";
       try{percentage=percentage.substring(0, 5);       }
       ...
       for(i=2;i<=5;i++){
           if(i==4){continue;}
           if(i==5){xpos-=18;} //for central alignment
       int j=0;
           for(j=0;j<rowNo;j++){               
           val=tab.getValueAt(j, i).toString();               
           if(val.equals("-100")){continue;}
           if(val.equals("-1")){val="[Ab]";}
           if(val.equals("-2")){val="[M]";}
           try{
       ...
       //srL = listLB1.getSelectedIndices();
            ...
            for(i=0;i<opSub.length;i++){
                ...
                String val="";
            for(int j=2;j<6;j++){
                if(j==5){xpos-=18;} //for central alignment
                if(j==4){continue;}
                val=tabl.getValueAt(num[i], j).toString();
                System.out.println("%%%%%%%%%%%%%I IS %%%%%%%%%"+i+"%%%%%%"+num[i]+"((((((((("+val);
       ...
       catch(Exception e){System.out.println("^^^^^^^^^^^^^^not not not^^^^^^^^^^^^^^^^^^^^^"+e);}
        
       
       //entering grand total for grades
       try{
       ...
       String val;
           ...
           val="ERROR";
           System.out.println("JA"+sum);
           if(gt==90){
               sum/=9;//converting to cgpa system
               }
               else{sum/=10;//converting to cgpa system
           }
           
           
               val=cgpa(sum);
           
           gd.drawString(val+"",930, saveForOpSubTotal+=cellwidth);
    ...
    private void initComponents() {
        ...
        jTextField1.setText("MORAL KNOWLEDGE PUBLIC SR. SEC. SCHOOL");
        jTextField1.setBorder(null);

        jLabel1.setIcon(new javax.swing.ImageIcon(getClass().getResource("/icon/mone.jpg"))); // NOI18N

        jTextField2.setFont(new java.awt.Font("Times New Roman", 1, 17)); // NOI18N
        ...
        sessTF.setFont(new java.awt.Font("Times New Roman", 1, 18)); // NOI18N
        sessTF.setHorizontalAlignment(javax.swing.JTextField.CENTER);
        sessTF.setText("Half Yearly Examination : 2016-17");
                        ...
                        .addGroup(jPanel3Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                            .addGroup(jPanel3Layout.createSequentialGroup()
                                ...
                                .addGroup(jPanel3Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.TRAILING)
                                    ...
                                    .addComponent(stIm, javax.swing.GroupLayout.PREFERRED_SIZE, 92, javax.swing.GroupLayout.PREFERRED_SIZE)))
                            .addGroup(jPanel3Layout.createSequentialGroup()
                ...
                .addGroup(jPanel3Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                    .addGroup(jPanel3Layout.createSequentialGroup()
                        .addGroup(jPanel3Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING, false)
                            ...
                            .addComponent(stIm, javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE))
                        .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
...
i++;
        ...
        enterMarks obj=new enterMarks();
        obj.sessionCB.setSelectedIndex(sessionCB.getSelectedIndex());
...
List<String> selectedValuesList = listLB.getSelectedValuesList();
    ...
    private void jButton2ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton2ActionPerformed
      ...
      //  int countSelectedStud= srL.length;  
       // countSelectedStud=1;
      //  JOptionPane.showMessageDialog(null, Arrays.toString(srL));
